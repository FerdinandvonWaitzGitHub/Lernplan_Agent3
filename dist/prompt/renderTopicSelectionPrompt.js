export const renderTopicSelectionPrompt = (payload, templates) => {
    // Alle verfügbaren Themen aus allen Templates sammeln
    const allTopics = templates.flatMap(template => template.entries.map(entry => ({
    ...
    return [
        ...
        payload.weights.map(({ legalArea, weight }) => `   - ${legalArea}: ca. ${Math.round(targetTotalTopics * weight)} Themen (${(weight * 100).toFixed(0)}%)`).join('\n'),
...
//# sourceMappingURL=renderTopicSelectionPrompt.js.map