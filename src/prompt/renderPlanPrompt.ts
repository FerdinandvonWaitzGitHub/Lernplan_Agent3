...
import type { TemplateEntry, TemplateSummary } from '../services/templateRegistry.js';

export const renderPlanPrompt = (payload: PlanRequestPayload, templates: TemplateSummary[]): string => {
    const summary = templates
        .map(template => {
            const preview = template.preview
                .map((entry: TemplateEntry) => [entry.legal_area, entry.sub_area, entry.topic_description].filter(Boolean).join(' - '))
                .filter(Boolean)
                .join('; ');
