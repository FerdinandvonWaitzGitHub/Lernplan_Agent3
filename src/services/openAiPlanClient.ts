...
export class OpenAiPlanClient {
    constructor(private readonly options: OpenAiPlanClientOptions) {
        if (!options.apiKey) {
    ...
    async generatePlan(prompt: string, overrides?: GeneratePlanOverrides): Promise<string> {
        ...
        const maxTokens =
            this.options.maxTokens && this.options.maxTokens > 0
